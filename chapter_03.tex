...
maximal $\mathfrak m$). Given a $\D_X$-module $\mathcal M$, we call it an \textbf{integrable connection} \phantomsection\label{def:integrable-connection}
if $\mathcal M$ is locally free as an $R$-module, the $R$ action is given by inclusion $R\subseteq \D_X$.
...
\begin{proof}
    ...
    \begin{align*}
        \partial_{x_j} \cdot \left( \sum f_i \cdot s_i \right) & = \sum [\partial_{x_j}, f_i] \cdot s_i + \sum f_i \cdot \partial_{x_j}(s_i)                                                              \\
                                                               & = \sum \partial_{x_j}(f_i) \cdot s_i + \sum f_i \cdot \partial_{x_j}(s_i)   &  & \text{(see \cite[Prop~2.25(iii)]{mustata2023dmodules})} \\
                                                               & = \sum g_i s_i = 0.
    \end{align*}
    ...
    \[
        \sum h_i s_i = 0 \quad \text{such that } d(h_1, \dots, h_{\ell}) = 0.
    \]
