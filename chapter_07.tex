...
\begin{definition}\label{def:convergent_spectral_sequence}
    We say the spectral sequence $\{E_{\bullet}^{\bullet}(k)\}_k$ $\textbf{converges to}$ $H^i(F^{\bullet})$ if for all $i,j$, there exists $k = k(i,j)$ such that $\forall k' \ge k$, $$E_j^i(k') \cong \mathrm{gr}_j^F H^i(F^{\bullet}).$$
\end{definition}
...
such that each $P^{i}_{\bullet}$ is a free graded $R_{\bullet}^{\Gamma}A$-module of finite rank.
...
\begin{proof}
    ...
    On the other hand, $N \cap \check{F}_{j}^{k+1}$ gives another good filtration. By Corollary \ref{cor:compare-good-filtrations}, there exists a uniform $\omega$ such that $$ \check{F}_{j}^{k+1} \cap \mathrm{d}(\check{F}^{k}) = N \cap \check{F}_{j}^{k+1} \subseteq N_{j+\omega} = \mathrm{d}(\check{F}_{j+\omega}^{k}), \quad \forall j. $$
\end{proof}
