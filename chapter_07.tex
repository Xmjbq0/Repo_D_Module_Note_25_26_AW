...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Since the filtration on $M$ is a good filtration, it is bounded from below. This implies the filtration on $\check{F}^\bullet$ is also bounded from below, so the spectral sequence converges (see Remark~\ref{rmk:classical_convergence_theorem}).
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item Since $F^k$ are free modules, taking the associated graded commutes with $\mathrm{Hom}$, i.e.,
              ...
              \]
              Since $F^k$ is a filtered free module (direct sum of shifted $A$), and for $A(s)$, the map $\Psi$ corresponds to the identity on $(\mathrm{gr}_{\bullet}A)(-s)$. Thus $\Psi$ is an isomorphism.
