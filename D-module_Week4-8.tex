...
\newtheorem{notation}[remark]{Notation} % 记号
...
\begin{definition}
    $\mathcal{L}$ is called a $\textbf{local system}$ if $\forall y \in Y$, $\exists$ open neighborhood $U_y$ of $y$ such that $\mathcal{L}|_{U_y} \simeq\mathbb{C}^r_{U_y}$, where $\mathbb{C}^{r}_{U_y}$ is the constant sheaf on $U_y$.
\end{definition}
...
\begin{proof}
    ...
    We have known $M$ is a locally free $R$-module, $\forall x \in X$, take an open neighborhood $U \subset X$ such that $M|_U$ is free, then $M^{\text{an}}|_{U^{\text{an}}}$ is a free $\mathcal{O}_{U^{\text{an}}}$-module, generated by $\underline{\mathfrak{m}} = (m_1, \cdots, m_r)$.
    ...
    Since $\Phi$ is invertible matrix, we obtain $s_{1},\dots,s_{r}$ also a basis of $U^{an}|_{V}$, so we have $M^{an}|_{V}=\bigoplus\limits_{i=1}^{r}\mathcal{O}_{V}s_{i}$ such that $\partial_{n}s_{i}=0$.
...
\begin{proof}
    ...
    For $k\ge 1$, we obtain that $F_{j-1}^{i}\subseteq F_{j+k-1}^{i}$$$E^{i}_{j}(k+1)=\frac{\{u\in F^{i}_{j}:du\in F^{i+1}_{j-k-1}\}+F^{i}_{j-1}}{F^{i}_{j}\cap d(F^{i-1}_{j+k})+F^{i}_{j-1}}.$$
        ...
        For $Z^{i}_{j}(k)+F^{i}_{j-1} /F^{i}_{j-1}\xrightarrow{d_{k}}Z_{j-k}^{i+1}(k)+F^{i+1}_{j-k-1} /F^{i+1}_{j-k-1}$, $\ker d_{k}:=\{u\in F^{i}_{j} /F_{j-1}^{i}: \exists x\in u\ such that dx\in F^{i+1}_{j-k-1}\}=Z^{i}_{j}(k+1)+F^{i}_{j-1} /F^{i}_{j-1}$. And $\mathrm{Im}(Z^{i-1}_{j+k}(k)+F^{i-1}_{j+k-1} /F^{i-1}_{j+k-1}\xrightarrow{d_{k}}Z^{i}_{j}(k)+F^{i}_{j-1}(k) /F^{i}_{j-1}(k))=d(Z^{i-1}_{j+k}(k))+F^{i}_{j-1}(k) /F^{i}_{j-1}(k)$, done.
\end{proof}
...
\begin{definition}
    We say the spectral sequence $\{E_{\bullet}^{\bullet}(k)\}_k$ $\textbf{converges to}$ $H^i(F^{\bullet})$ if for all $i,j$, there exists $k \gg 0$ such that $$E_j^i(k) \cong \text{gr}_j^F H^i(F^{\bullet}).$$
\end{definition}
...
Let $M$ be a finitely-generated $A$-module, and $\Omega_{\bullet}M$ a good filtration on $M$ such that $\mathrm{gr}^{\Omega} M \neq 0$.
Since $\mathrm{gr}^{\Gamma} A$ is noetherian, $R^{\Omega} M$ has a graded free resolution: $$\dots\rightarrow P_{\bullet}^{-1}\rightarrow P^{0}_{\bullet}\rightarrow R^{\Omega} _{\bullet}M\rightarrow0$$such that each $P^{i}_{\bullet}$ is a free graded $R_{\bullet}^{\Gamma}A$-module of finite rank.
...
\begin{proof}
    ...
    On the other hand, $N \cap \check{F}_{j}^{k+1}$ gives another good filtration. By Corollary 3.4, there exists a uniform $\omega$ such that $$ \check{F}_{j}^{k+1} \cap d(\check{F}^{k}) = N \cap \check{F}_{j}^{k+1} \subseteq N_{j+\omega} = d(\check{F}_{j+\omega}^{k}), \quad \forall j. $$
\end{proof}
...
Take $I^j =\begin{cases}\left< t \right>^{j},& j > 0\\R,&j\leq0\end{cases}$, we define a filtration of $\mathcal{D}_{X}$: $$V_{k} \mathcal{D}_X = \{P \in \mathcal{D}_X \mid P I^{j} \subseteq I^{j-k}\text{ for all }j\in \mathbb{Z}\}.$$Then we obtain that $V_0 \mathcal{D}_X = R \langle t\partial_t, \partial_{x_2}, \ldots, \partial_{x_n} \rangle$ and for all $i>0$: $$V_{-i} \mathcal{D}_{X} = t^{i} V_{0 }\mathcal{D}_{X }= V_{0} \mathcal{D}_{X} t^{i}\atop V_i \mathcal{D}_X = \partial_t V_{i-1} \mathcal{D}_X + V_{i-1} \mathcal{D}_X.$$Next, we will prove $V_{\bullet} \mathcal{D}_X$ is a noetherian $\mathbb{Z}$-filtration such that $\mathrm{gr}^V \mathcal{D}_X$ is Auslander regular.
...
and $\mathbb{C}[u] \hookrightarrow R_{\bullet}(\mathcal{O}_X)=R[u]\bigoplus\limits_{j\ge 1}I^{j}\otimes u^{-j}$ induces a family $\widetilde{X}= \operatorname{Spec} R_{\bullet}(\mathcal{O}_X)\xrightarrow{\varphi}\mathbb{A}^{1}_{\mathbb{C}}$ such that
\begin{enumerate}
...
\begin{theorem}[Björk]\label{Theorem 8.4}
    Let $M$ be a holonomic $\mathcal{D}_X$-module with a good filtration $\Omega_{\bullet} M$ over $V_{\bullet} \mathcal{D}_X$. There exists $b(s) \in \mathbb{C}[s]$ such that $$b(t\partial_t - k) \text{ kills } \frac{\Omega_k M}{\Omega_{k-1} M}, \quad \forall k \in \mathbb{Z}$$The monic minimal degree $b(s)$ is called the $\textbf{b-function}$ of $M$ along $Y$.
\end{theorem}
\begin{lemma}\label{lemma 8.5}
    Let $Y$ be a smooth variety over $\mathbb{C}$, $N$ a holonomic $\mathcal{D}_Y$-module, and $\theta\in End_{\mathcal{D}_Y}(N)$; then there exists $b(s)\in\mathbb{C}[s]$ such that $b(\theta)=0$.
\end{lemma}
\begin{proof}

    Since $N$ is holomonic, we obtain that$\mathrm{ch}(N) = \bigcup\limits_{i} T^*_{Y_i} Y$, with $Y_{i}$ locally closed. Take a $Y_i$ of maximal dimension and take open subset $U \subset Y$ such that $Y_{i}^{\circ} \hookrightarrow U$ a closed embedding such that $\overline{Y_i^{\circ}} = Y_{i}$ and $U \cap \{\bigcup\limits_{j\neq i}Y_j\} = \varnothing$, then $\mathrm{ch}(N|_U) = T^*_{Y_i^\circ} Y\Rightarrow N|_U = \mathcal{O}_{Y_i^\circ} \otimes L[\partial_1, \ldots, \partial_k]$ with $L$ a local system on $Y_i^\circ$. We have $\mathrm{End}(N|_U) = \mathrm{End}_{\mathbb{C}}(L)$, therefore, $\exists b_i(s) \in \mathbb{C}[s]  \text{ such that } b_i(\theta|_{U}) = 0.$
...
\begin{proof}[proof of (\ref{Theorem 8.4})]
    ...
    Now, take $\theta=\bigoplus \limits_{i}(t\partial_{t}-i),$ then $\theta\in\mathrm{End}_{\mathcal{D}_{T_{Y}X}}(\mathrm{gr}_{\bullet}^\Omega M)$. By Lemma \ref{lemma 8.5}, there exists $b(s)\in\mathbb{C}[s]$ such that $b(\theta)=0$, In particular, $b(t\partial_{t}-k)$ kills $\frac{\Omega_{k}M}{\Omega_{k-1}M}$ for all $k$.
\end{proof}
...
\begin{theorem}[Kashiwara-Malgrange]\label{Theorem 8.7}
    Let $M$ be a holonomic $\mathcal{D}_{X}$-module, there exists unique good filtration $V_{\bullet}M$ and $b(s)\in\mathbb{C}[s]$ such that
    \begin{enumerate}
